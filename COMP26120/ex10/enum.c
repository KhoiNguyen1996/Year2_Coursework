...
void enumerate()
{
  ...
  QUIET=1;
  best_value=0;
  float noOfSolution =  pow(2, Nitems);
  printf("Current progress:      [");
 while(!(next_binary(&solution[1], Nitems)))
    {
        progress++;

      if (fmod(progress/noOfSolution * 100 , 2.5))
      {
        printf("Current Progress: %lf\n", progress/noOfSolution * 100);
        printf("[");

        for (i=0; i<progress/noOfSolution * 100; i = i+2.5)
          printf("*");
        printf("]");
        printf("\n");
      }
      ...
      if (infeasible == 0 && best_value < total_value)
      {
        best_value = total_value;
        // Copy the current solution to best solution.
      ...
      printf("Current Progress: Completed\n");
      printf("[");
      for (i=0; i<progress/noOfSolution * 100; i = i+2.5)
        printf("*");
      printf("]");
 ...
 /* ADD CODE TO PRINT OUT BEST SOLUTION */
      ...
      // Function to print out the best solution.
      for (i=1;i < Nitems+1;i++)
      {
