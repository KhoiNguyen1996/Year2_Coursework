...
void frac_bound(struc_sol *sol, int fix)
{
  ...
  do
    {
      totalw+=item_weights[temp_indexes[i]];struc_sol *pqueue;
      totalp+=item_values[temp_indexes[i]];
      i++;
...
void branch_and_bound(int *final_sol)
{
  ...
  struc_sol *final_solution;
  copy_array(final_sol, &final_solution->solution_vec[1]);
  int i;
  for (i=1; i<= Nitems; i++)
  {
    if (strcmp(final_solution->solution_vec[i], "*") != 0)
      final_solution->fixed++;
  }
  // compute its value and its bound
  frac_bound(final_solution, final_solution->fixed);

  // put current_best = to its value
  pqueue->val = final_solution->val;

  // store it in the priority queue
  insert(*final_solution);
