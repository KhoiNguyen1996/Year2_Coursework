...
// Function to sort the list with respect to user response using qsort.
void qSortField(int N, char chosenField[])
{
  if (strcmp(chosenField, "price") == 0)
...
void user_interface(int N)
{
  ...
  if (strcmp(userReply, "yes") == 0)
  {
    ...
    // merge sort.
    qSortField(N, userLastField);
    qSortField(N, userSecondField);
    qSortField(N, userFirstField);
  }
}
