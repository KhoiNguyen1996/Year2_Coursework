...
B *list;
char column1[50], column2[50], column3[50], column4[50];
...
void print_results(int N)
{
    ...
    if((fp=fopen("top20.txt","w")))
    {
      for(i=N-1;i>=N-20;i--)
      {
	  printf("%s: %g|%s: %g|%s: %g|%s: %d\n", column1, list[i].rating, column2, list[i].price, column3, list[i].relevance, column4, list[i].ID);
	  fprintf(fp, "%s: %g|%s: %g|%s: %g|%s: %d\n", column1, list[i].rating, column2, list[i].price, column3, list[i].relevance, column4, list[i].ID);
