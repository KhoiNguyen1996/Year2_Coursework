...
def reservation_request(url, slot_ID, printText):
...
<slot_id>&&slot_ID&&</slot_id>
    ...
    if request_result.status_code == 200:
        ...
        return reservation_request(url, slot_ID, 0)

#===============================================================================

# Function to request a reservation for the clinet.
def cancel_reservation(url, slot_ID, printText):
    # Get current time as the request ID, as it's always unique.
    current_ID = int(getTime() * 100)
...
<slot_id>&&slot_ID&&</slot_id>
    ...
    if request_result.status_code == 200:
        ...
        if server_result.status_code != 200 and printText == 1:
            print "There was a problem processing the request"
            print "Processing again..."
        ...
        cancel_reservation(url, slot_ID, 0)
...
def get_slots():
    ...
    if not (band_free_slot is None or hotel_free_slot is None):
...
def start_client():
    ...
    while True:
        ...
        if choice == '0':
            ...
            current_slot = get_bookings(url_band)

            slot_list = get_slots()
            count = 0
            while len(slot_list) == 0 and count < 5:
                slot_list = get_slots()
                count = count + 1
            ...
            if len(slot_list) == 0:
                ...
                if current_slot is not None:
                    # Cancel later reservation to reserve best slot.
                    for i in range(len(current_slot) - 1, -1, -1):
                        if current_slot[i] > slot_ID:
                            cancel_reservation(url_band, current_slot[i], 0)
                            cancel_reservation(url_hotel, current_slot[i], 0)
                            isSend = isSend + 1

                if isSend > 0:
                    # Send request to band server.
                    stage_one = reservation_request(url_band, slot_ID, 1)
                    time.sleep(wait_time)
                    ...
                    if stage_one == -1:
                        ...
                        stage_two = reservation_request(url_hotel, slot_ID, 0)
                    ...
                    print "========================================================"
                    print "     There are currently no better available slots"
                    print "                    Please try again"
                    print "========================================================"
            ...
            cancel_reservation(url_band, slot_ID, 0)
            cancel_reservation(url_hotel, slot_ID, 0)
        elif choice == '3':
            ...
            count = 0
            while len(available_slots) == 0 and count < 5:
                available_slots = get_slots()
                count = count + 1
            for slot in available_slots:
                print "Slots Available:", slot
        else:
