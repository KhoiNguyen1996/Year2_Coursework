...
// Variable to represent the state of the view; jumping, standing or falling.
int isJumping = 0;
...
void calculate_lookpoint(void) { /* Given an eyepoint and latitude and longitude angles, will
  ...
  double lat_RAD = (lat + mlat) * DEG_TO_RAD;
  double lon_RAD = (lon + mlon) * DEG_TO_RAD;

  double dir_x = cos(lat_RAD) * sin(lon_RAD);
  double dir_y = sin(lat_RAD);
  double dir_z = cos(lat_RAD) * cos(lon_RAD);
...
void spin(void) {
  ...
  if (isJumping == 1)
  {
    eyey += 0.1;
    centery = centery + 0.1;
    if (eyey > jumpLimit) {isJumping = 0;}
  }
  else if (isJumping == 0)
  {
    eyey -= 0.1;
    centery -= 0.1;
    if (eyey <= 1.7) {isJumping = 3;}
  }
...
void mouse_motion(int x, int y) {
  ...
  x = (GLdouble) x, y = (GLdouble) y;

  GLdouble x_Offset = -(x * 100.0 / height - 50);
  GLdouble y_Offset = -(y * 100.0 / height - 50);
...
void keyboard(unsigned char key, int x, int y) {
  ...
  switch (key) {
    case 27:  /* Escape key */
      ...
      break;
    case '.': // Comma
      eyex += RUN_SPEED * sin((lon + mlon - 90) * DEG_TO_RAD);
      eyez += RUN_SPEED * cos((lon + mlon - 90) * DEG_TO_RAD);
      break;
    case ',': // Full-stop
      eyex += RUN_SPEED * sin((lon + mlon + 90) * DEG_TO_RAD);
      eyez += RUN_SPEED * cos((lon + mlon + 90) * DEG_TO_RAD);
      break;
    case 'c': // Key c for crouching
      if (eyey >= 1.7)
        eyey -= 0.5;
      else
        eyey = 1.7;
